    ...
    public static ParseTemplate parse(Template _template) {
        ...
        for (JsonTable table : _template.getTableList()) {
            ...
            for (JsonTable.Column column : table.getInsert()) {
                /*
                 * getAndCreateIfNeed( OpType.ADD, opTypeFieldSetMap,
                 * ArrayList::new) .add(column.getColumn());
                 */

                opTypeFieldSetMap.computeIfAbsent(OpType.ADD, k -> new ArrayList<>()).add(column.getColumn());
            }

            for (JsonTable.Column column : table.getUpdate()) {
                opTypeFieldSetMap.computeIfAbsent(OpType.UPDATE, k -> new ArrayList<>()).add(column.getColumn());
            }

            for (JsonTable.Column column : table.getDelete()) {

                opTypeFieldSetMap.computeIfAbsent(OpType.DELETE, k -> new ArrayList<>()).add(column.getColumn());
            }
        }
